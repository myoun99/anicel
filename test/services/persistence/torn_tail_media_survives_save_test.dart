...
void main() {
  ...
  test('an overlay over an UNREADABLE base says so — not "different '
      'version"', () async {
    const service = AnicelFileService();
    final path = '${directory.path}/base.anicel';
    final store = BrushFrameStore();
    store.storeBakedSurface(key('f1'), inked(3));
    ...
    final overlayPath = '${directory.path}/base.overlay';
    store.storeBakedSurface(key('f1'), inked(9));
    await service.writeRecoveryOverlay(
      project: project,
      brushFrameStore: store,
    ...
    // The base tears AFTER the overlay was stamped. Nothing was ever
    // compared — lumping this in with "made from a different version"
    // sent the user hunting for a versioning mistake nobody made.
    final healthy = parseAnicelZipLayoutFile(path);
    File(path).openSync(mode: FileMode.append)
      ..truncateSync(healthy.centralDirectoryOffset + 7)
      ..closeSync();
    ...
    await expectLater(
      service.open(filePath: path, overlayPath: overlayPath),
      throwsA(
        predicate(
          (error) =>
              error is FormatException &&
              error.message.contains('cannot be read right now'),
          'the unreadable-base message, not the version-mismatch one',
        ),
      ),
    );
  });
