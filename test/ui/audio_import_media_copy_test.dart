...
void main() {
  ...
  EditorSessionManager sessionWithFakeConforms([Project? project]) =>
  ...
  test('the carry answer survives a save and reopen', () async {
    ...
    final reopened = await openedSession(path, make: sessionWithFakeConforms);
  ...
  test('an open WARMS the sounds the project plays, so the first play is not '
      'the decode', () async {
    final session = sessionWithFakeConforms();
    final path = '${directory.path}/scene.anicel';
    final sound = File('${directory.path}/guide.wav')
      ..writeAsBytesSync(List<int>.filled(40, 3));
    ...
    expect(
      warmed.paths,
      contains(reopened.mediaPool.mediaAssets.single.path),
    );
  });
}

/// A conform store that says which sources it was asked to warm.
class _WarmRecorder extends AudioConformStore {
  _WarmRecorder()
    : super(
        resolveConformPath: (_) => null,
        runner: (request) async => const ConformResult(
