...
void main() {
  ...
  group('the conte page', () {
    ...
    test('🚨a cut band and the page total follow the drag; with no drag they '
        'are the labels the layout built', () {
      ...
      final band = page.cutBands.firstWhere((band) => band.cutId == 'c0');
      expect(
        band.showsLength,
        isTrue,
        reason: 'fixture premise: the first cut ends on this page',
      );

      expect(painter.lengthLabelFor(band), '0+6');
      expect(painter.footerTotalLabel(), '0+12');

      // The first cut is dragged out to thirty frames.
      channel.value = CutTrimDragPreview(
        previewDurations: {const CutId('c0'): 30},
      );
      expect(painter.lengthLabelFor(band), '1+6');
      expect(
        painter.footerTotalLabel(),
        '1+12',
        reason: 'the running total is the lengths ending here, read live',
      );
      expect(
        page.cutBands.firstWhere((band) => band.cutId == 'c1').lengthLabel,
        '0+6',
        reason: 'the LAYOUT stays as it was built — only the paint follows',
      );
    });

    test('a page with no drag channel prints its built labels', () {
      final source = buildConteSheetSource(project());
      final page = layoutConteSheet(source).first;
      final painter = ContePagePainter(page: page, source: source);
      final band = page.cutBands.firstWhere((band) => band.cutId == 'c0');

      expect(painter.lengthLabelFor(band), band.lengthLabel);
      expect(painter.footerTotalLabel(), page.pageTotalLabel);
    });
  });
}
