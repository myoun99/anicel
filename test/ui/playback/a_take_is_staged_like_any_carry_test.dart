...
void main() {
  ...
  test('🚨a take is conformed from its own bytes — asked once the pool '
      'records it, not before', () async {
    late EditorSessionManager manager;
    // What each conform was handed to read — a take has no file of its own,
    // so the only bytes it can be are the carry the pool names.
    final read = <List<int>?>[];
    final conforms = AudioConformStore(
      resolveConformPath: (_) => null,
      resolveByteSource: (path) => manager.projectFile.mediaByteSourceFor(path),
      resolveCarry: (path) => manager.projectFile.mediaCarryFor(path),
      runner: (request) async {
        final source = request.source;
        read.add(
          source != null && source.existsSync() ? source.readSync() : null,
        );
    ...
    await conforms.ensureFor(manager.mediaPool.mediaAssets.single.path);

    expect(
      read,
      hasLength(1),
      reason:
          'asked before the pool named its carry, the conform read an '
          'address with nothing at it — and ran again once the carry '
          'appeared',
    );
    expect(
      read.single,
      allOf(isNotNull, isNotEmpty),
      reason: 'read the take\'s own bytes, the staged copy',
    );
  });

  test('🚨a take taken out of the pool keeps its name while an undo can '
      'bring it back — the next one walks past it', () async {
    ...
    Future<String> record() async {
      ...
      return manager.mediaPool.mediaAssets.single.path;
    }

    final first = await record();
    expect(manager.mediaPool.removeMediaAsset(first), isTrue);
    final second = await record();

    expect(
      second,
      isNot(first),
      reason: 'the pool had forgotten it, and the room still holds its copy',
    );
    manager.undo(); // the second take
    manager.undo(); // the removal
    expect(manager.mediaPool.mediaAssets.map((asset) => asset.path), [first]);
  });
