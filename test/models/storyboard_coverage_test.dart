...
void main() {
  ...
  group('re-timing the lead (feedback #5)', () {
    test('a shrink shortens the FIRST cell and every later entry comes '
        'along keeping its own comma', () {
      final moved = storyboardTimelineWithLeadRetimed(
        timeline: _timeline({
          0: _drawing('a', length: 5),
          5: _drawing('b', length: 5),
        }),
        cutDuration: 10,
        delta: 2,
      )!;

      expect(moved.keys, [0, 3]);
      expect(moved[0]!.length, 3);
      expect(moved[3]!.length, 5);
    });

    test('the first cell keeps one frame — the shrink clamps there', () {
      expect(
        storyboardLeadRetimeMaxShrink(
          timeline: _timeline({
            0: _drawing('a', length: 5),
            5: _drawing('b', length: 5),
          }),
      ...
      expect(moved.keys, [0, 1]);
      expect(moved[0]!.length, 1);
      expect(moved[1]!.length, 5);
    });

    test('a grow is the same move backward — unbounded, the cut end '
        'absorbs it', () {
      final moved = storyboardTimelineWithLeadRetimed(
        timeline: _timeline({
          0: _drawing('a', length: 5),
      ...
      expect(moved.keys, [0, 8]);
      expect(moved[0]!.length, 8);
      expect(moved[8]!.length, 5);
    });

    test('one panel is the degenerate case: only the comma changes', () {
      final moved = storyboardTimelineWithLeadRetimed(
        timeline: _timeline({0: _drawing('a', length: 10)}),
        cutDuration: 10,
        delta: 3,
      )!;

      expect(moved.keys, [0]);
      expect(moved[0]!.length, 7);
    });

    test('data past the cut end rides too, so its distance to the end '
        'stays what it was', () {
      final moved = storyboardTimelineWithLeadRetimed(
        timeline: _timeline({
          0: _drawing('a', length: 5),
          5: _drawing('b', length: 5),
          12: _drawing('c', length: 2),
        }),
        cutDuration: 10,
        delta: 2,
      )!;

      expect(moved.keys, [0, 3, 10]);
    });

    test('nothing to re-time (no drawing inside the cut, or a zero delta) '
        'is null', () {
      expect(
        storyboardTimelineWithLeadRetimed(
          timeline: _timeline({}),
          cutDuration: 10,
          delta: 2,
        ),
      ...
      expect(
        storyboardLeadRetimeMaxShrink(timeline: null, cutDuration: 10),
        isNull,
      );
      expect(
        storyboardTimelineWithLeadRetimed(
    ...
    test('a shrink drops the first cell\'s inbetween dots it cut off', () {
      final moved = storyboardTimelineWithLeadRetimed(
        timeline: _timeline({
          0: TimelineExposure.drawing(
            const FrameId('a'),
      ...
      expect(moved[0]!.length, 3);
      expect(moved[0]!.breakdownOffsets, [2]);
    });
  });
}
