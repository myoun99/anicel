  ...
  UIPencilInteractionDelegate
{
  ...
  func didInitializeImplicitFlutterEngine(_ engineBridge: FlutterImplicitEngineBridge) {
    ...
    storageChannel.setMethodCallHandler { call, result in
      ...
      case "readFileCoordinated":
        let arguments = call.arguments as? [String: Any]
        AppDelegate.readFileCoordinated(
          sourcePath: arguments?["sourcePath"] as? String,
          destinationPath: arguments?["destinationPath"] as? String,
          result: result)
      case "requestFileDownload":
        let arguments = call.arguments as? [String: Any]
        AppDelegate.requestFileDownload(
          sourcePath: arguments?["sourcePath"] as? String, result: result)
      case "readInPlaceCoordinated":
        let arguments = call.arguments as? [String: Any]
        AppDelegate.readInPlaceCoordinated(
          sourcePath: arguments?["sourcePath"] as? String, result: result)
  ...
  /// The READ twin of the above, for the same reason: a File Provider
  /// document (Drive, Dropbox…) can be a non-materialised placeholder,
  /// and a plain `dart:io` read of one fails even inside an open
  /// security scope. Coordinated reading is what tells the provider to
  /// download; the bytes are staged into [destinationPath] (the app's
  /// own tmp), which Dart then reads like any local file.
  ...
  /// Asks the provider for [sourcePath]'s CURRENT content and copies
  /// nothing: the item is opened inside a coordinated read and closed
  /// again, and the Dart caller then reads it where it lies.
  ///
  /// 🚨THE COORDINATED READ IS THE ASK. `requestFileDownload` above is
  /// iCloud's API and every other File Provider ignores it, so an iPad
  /// opening a Drive project in place read the copy Drive had cached from
  /// its own first save (2026-09-13: one cut of twelve, while the desktop's
  /// revision sat in the cloud). Coordination is what the system turns
  /// into the provider's fetch, and the only lever an app has. Blocks
  /// until the provider yields, which is why it runs off the main thread.
  ...
  /// Tells the coordinator — and through it a File Provider — that
  /// [sourcePath]'s content changed: a coordinated write whose block only
  /// sets the modification date. The app appended in place with plain
  /// `dart:io`, which a provider never sees; the coordination is the
  /// message (M-1: strokes on disk, nothing uploaded). A local file, with
  /// no presenter and no provider, pays for nothing but the call.
