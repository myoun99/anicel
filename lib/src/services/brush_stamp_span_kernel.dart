...
/// re-checks.
///
/// Shared at SPAN granularity, exactly like `blendDabTilesDart`: one call
/// per (row, tile) run of pixels, nothing per pixel became indirect, and
/// [dabOpacity] and [erase] are hoisted out of the loop, so this costs
/// nothing per pixel even in a debug build, where nothing inlines.
///
/// [buffer] is written IN PLACE from [offset]; [rgba] is read from
/// [sourceOffset]; [count] pixels. Returns whether any byte changed —
/// the commit turns that into one dirty-tile mark per span instead of one
/// per pixel.
///
/// The C kernel is a THIRD transcription and stays one: it is a different
/// language, gated behind the ABI version, and pinned byte-exact by its
/// own parity suite. It already has this exact shape —
/// `qa_stamp_blend_row(tile_row, stamp_row, count, opacity, erase)`
/// returning whether the row changed — which is where the signature here
/// comes from.
bool blendStampSpanInPlace(
  Uint8List buffer,
  int offset,
  Uint8List rgba,
  int sourceOffset,
