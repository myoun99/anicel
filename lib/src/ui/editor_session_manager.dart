...
class EditorSessionManager extends ChangeNotifier {
  ...
  bool beginAutoFrameForStroke() {
    ...
    _frameSequence += 1;
    final command = _timelineController.createDrawingFrameCommandForLayer(
      layerId: layer.id,
      frameId: FrameId(_nextFrameId(layer.id)),
    );
    command.execute();
  ...
  void _pasteRun({
    ...
    for (var i = 0; i < targets.length; i += 1) {
      ...
      runs.add((
        ...
        index: replacing
            ? _commitBlockStart(target.id, selection.startIndex)
            : index,
        liftCount: liftCount,
        clip: placed.clip,
  ...
  void scrubGlobalFrame(int globalFrame) {
    ...
    if (axis.isGap(globalFrame) ||
      ...
      if (frameScrubActive.value && !scrubOutOfTerritory.value) {
        scrubOutOfTerritory.value = true;
      }
      return;
    }
